...
public class FirestormSwordItem extends SwordItem {
    public FirestormSwordItem(Tier tier, int attackDamage, float attackSpeed, Properties properties) {
    ...
    public boolean hurtEnemy(ItemStack itemStack, LivingEntity target, LivingEntity attacker) {
        ...
        if (result && !target.getLevel().isClientSide()) {
            if (target.isOnFire()) {
                target.hurt(DamageSource.mobAttack(attacker), 2.0F);
            }
